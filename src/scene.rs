...
const CUBE_EMISSIVE_STRENGTH: f32 = 0.9;
...
pub fn build_scene(cube_texture: Rc<Texture>) -> Vec<Cube> {
    let main_cube_material = Material::textured(cube_texture, CUBE_EMISSIVE_STRENGTH);
...
mod tests {
    ...
    fn test_texture() -> Rc<Texture> {
        Rc::new(Texture::from_pixels(1, 1, vec![Color::new(1.0, 1.0, 1.0)]))
    }

    #[test]
    fn scene_contains_the_main_cube_and_its_landscape() {
        let cubes = build_scene(test_texture());
        assert_eq!(cubes.len(), 1 + 1 + 5 + 3 + 4);
    }
    ...
    #[test]
    fn main_cube_uses_the_provided_texture() {
        let cubes = build_scene(test_texture());
        let main_cube = &cubes[1];
        assert!(main_cube.material.texture.is_some());
    }
}
